...
export function createAgentApp(options: ServerHandlerOptions): Hono {
  ...
  // Agent page (root)
  app.get("/", (c) => c.html(renderAgentPage(options.agent)));
...
function handleWebSocket(
  ...
  ws.onopen = async () => {
    ...
    session = new VoiceSession(sessionId, ws, options.agent, deps);
    sessions.set(sessionId, session);

    log.info(
      { sid, tools: options.agent.tools.size },
      "Session configured",
    );
