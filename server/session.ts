    ...
    this.deps = {
      ...
      config: {
        ...
        sttConfig: {
          ...
          ...(agent.config.prompt ? { prompt: agent.config.prompt } : {}),
        },
        ttsConfig: {
          ...deps.config.ttsConfig,
          ...(agent.config.voice ? { voice: agent.config.voice } : {}),
        },
      },
    };

    this.toolSchemas = [
      ...agentToolsToSchemas(agent.tools),
      ...getBuiltinToolSchemas(agent.config.builtinTools ?? []),
    ];
