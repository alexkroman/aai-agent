...
describe("ERR (browser-facing errors)", () => {
  it("has correct error strings", () => {
    ...
    expect(ERR.INVALID_CONFIGURE).toBe(
      "First message must be a valid configure message",
    );
    expect(ERR.STT_CONNECT_FAILED).toBe(
      "Failed to connect to speech recognition",
    );
...
describe("ERR_INTERNAL (server-side errors)", () => {
  ...
  it("TOOL_UNKNOWN formats tool name", () => {
    expect(ERR_INTERNAL.TOOL_UNKNOWN("my_tool")).toBe(
