// _test-utils.ts — Shared test helpers for SDK tests.
...
/** Create a mock IToolExecutor. */
export function createMockToolExecutor(): IToolExecutor & {
  executeCalls: { name: string; args: Record<string, unknown> }[];
...
export function createMockSessionDeps(overrides?: Partial<SessionDeps>): {
  ...
  return { deps, sttHandle, ttsClient, toolExecutor, llmCalls, llmResponses };
}
